...
class NotificationController extends Controller
{
        ...
        return Datatables::of($notifications)
            ...
            ->addColumn('seen',function ($notifications){
              ...
              ->addColumn('created_at',function ($notifications){
                return date('d/m/Y',strtotime($notifications->created_at));
            })
     ...
     public function unavailableData(Request $request)
    {
        ...
        return Datatables::of($notifications)
            ...
            ->addColumn('created_at',function ($notifications){
                return date('d/m/Y H:i:s',strtotime($notifications->created_at));
            })
            ->addColumn('action',function ($notifications){
                $data = json_decode($notifications->data,true);
      ...
      public function addressUpdateData(Request $request)
    {
        ...
        return Datatables::of($notifications)
            ...
            ->addColumn('description',function ($notifications){
                $data = json_decode($notifications->data,true);
            ...
            ->addColumn('created_at',function ($notifications){
                $data = json_decode($notifications->data,true);
                return isset($data['created_at']['date']) ? date('d/m/Y H:i:s',strtotime($data['created_at']['date'])) : '';
      ...
      public function addressDetails(Request $request)
    {
        ...
        if(isset($data['type']) && $data['type'] =="out of stock" ||isset($data['type']) && $data['type'] =="update"){
            $vendor_product = VendorProduct::where('user_id',$data['vendor_id'])->where('product_id',$data['product_id'])->whereNull('deleted_at')->first();
            if(isset($vendor_product)){
