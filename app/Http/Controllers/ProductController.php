...
class ProductController extends Controller
{
  ...
  public function productlisting(Request $request, $slug)
  {
    ...
    if (Auth::user()) {
      $vendorProduct =  $this->vendorProduct->with(['product', 'product.MeasurementClass',  'product.image', 'cart' => function ($q) use ($zone_id) {
        ...
        $q->where('status', '=', '1');
      })->where('status', '1')->whereIn('user_id', $user_id_array);
    } else {
      $vendorProduct =  $this->vendorProduct->with(['product', 'product.MeasurementClass',  'product.image'])->whereHas('product', function ($q) {
        $q->where('status', '=', '1');
      })->where('status', '1')->whereIn('user_id', $user_id_array);
    }
    $parent_data = [];
    ...
    if (!empty($vendorProduct)) {
      ...
      foreach ($vendorProduct['data'] as $vprec) {
        $rec =  $vprec;
        $rec['cart'] = isset($vprec['cart']) ? $vprec['cart'] : '';
        $rec['wish_list'] = isset($vprec['wish_list']) ? $vprec['wish_list'] : '';
        $rec['is_offer'] = false;
        // $rec['offer_id'] = null;
        $rec['price'] = number_format($vprec['best_price'], 2, '.', '');
        $rec['mrp'] = number_format(!empty($vprec['price']) ? $vprec['price'] : $vprec['best_price'], 2, '.', '');
        $rec['offer_price'] = number_format($vprec['best_price'], 2, '.', '');
        $rec['offer_data'] =  $ffer_data = $this->offer->where('id', $vprec['offer_id'])->where('from_time', '<=', date('Y-m-d'))->where('to_time', '>=', date('Y-m-d'))->first();

        $discount = ($vprec['price'] - $vprec['best_price']) / $vprec['price'];
        ...
        if (!empty($ffer_data)) {
          $rec['is_offer'] = true;
          $rec['offer_id'] = $vprec['offer_id'];
          $rec['offer_data'] = $ffer_data;
          if ($ffer_data->offer_type == 'amount') {
            $vprec['offer_price'] = $vprec['best_price'] - $ffer_data->offer_value;
          } else if ($ffer_data->offer_type == 'percentages') {
            $vprec['offer_price'] = $vprec['best_price'] - ($vprec['best_price'] * ($ffer_data->offer_value / 100));
          }
          //$rec['price'] =
          $rec['offer_price'] = number_format($vprec['offer_price'], 2, '.', '');
        } else {
          $rec['price'] = number_format($vprec['best_price'], 2, '.', '');
          $rec['mrp'] = number_format(!empty($vprec['price']) ? $vprec['price'] : $vprec['best_price'], 2, '.', '');
          $rec['offer_price'] = number_format($vprec['best_price'], 2, '.', '');
    ...
    if ($slug && !empty($slug)) {
      return view('pages.productlisting', ['products' => $vendorProduct, 'slug' => $slug, 'vProduct' => $vProduct, 'parent_data' => $parent_data, 'zone_id' => $zone_id, 'categoryAds' => $categoryAds, 'category_data' => $category_data]);
    } else {
      return view('pages.productlisting', ['products' => $vendorProduct, 'slug' => 'all', 'vProduct' => $vProduct, 'parent_data' => $parent_data, 'zone_id' => $zone_id, 'categoryAds' => $categoryAds]);
    }
  }
  ...
  public function productdeatils(Request $request, $id)
  {
    ...
    if ($Product->Product) {
      foreach ($Product->Product->related_products as $related_product) {
        ...
        if (Auth::user()) {
          ...
          $Product_related_products[] = $this->vendorProduct->with([
    ...
    if (!empty($Product)) {
      ...
      $products['price'] = number_format($Product->best_price, 2, '.', '');
      $products['mrp'] = number_format(!empty($Product->price) ? $Product->price : $Product->best_price, 2, '.', '');
      $products['offer_price'] = number_format($Product->best_price, 2, '.', '');
      ...
      $discount = ($Product->price - $Product->best_price) / $Product->price;
      $discount = $discount * 100;
      ...
      if (!empty($Product->offer_id)) {
        ...
        if (!empty($offer_data)) {
          ...
          if ($offer_data->offer_type == 'amount') {
            $products['offer_price'] = $Product->best_price - $offer_data->offer_value;
          } else if ($offer_data->offer_type == 'percentages') {
            $products['offer_price'] = $Product->best_price - ($Product->best_price * ($offer_data->offer_value / 100));
          }
          $products['offer_price'] = number_format($products['offer_price'], 2, '.', '');
        } else {
          $products['offer_price'] =  $products['price'] = number_format($Product->best_price, 2, '.', '');
        }
