    ...
    **/
    ...
    public function login(Request $request){
        ...
        if ($validator->fails()) {
            ...
            //return $user;
            if (empty($user)) {
                    return response()->json(['code' => 0, 'error' => true, 'message' => 'User not exist'], 200);
            }  
            if ($user) {
    ...
    public function resend_otp(Request $request) {
        ...
        if ($request->resend_type == 1) {
            $otp_details = DB::table('users')->where('id', $request->user_id)->first();
            //print_r(expression)
        ...
        $response_data['code'] = 1;
        $response_data['error'] = false;
        $response_data['data'] =$otp_details;
        return response()->json($response_data, 201);
    }
        ...
        foreach($adate_array as $dataarray){
            ...
            $ordersnew = $ordersnew->where('order_status','!=','D')->where('order_status','!=','C')->where('order_status','!=','R');
               ...
               foreach($ordersnew as $key=>$newdata){
                    ...
                    $arraylist['sub_total'] = $newdata->total_amount;
                    $arraylist['shipping_price'] = $newdata->delivery_charge;
                    $arraylist['price'] = $newdata->total_amount + $newdata->delivery_charge;      
                    ...
                    //$neworderList1['dilevery_date'][]=$arraylist;
                     //usort($arraylist, array($this,'sortByDistance')) ;
                        if(is_array($arraylistA) && !empty($arraylistA)){
                            ...
                            if(in_array($newdata->delivery_time->from_time."-".$newdata->delivery_time->to_time, $refrenceArray)){
                                ...
                                $arraylistA[$found_key]['time_name']   = (!empty($newdata->delivery_time))?$newdata->delivery_time->from_time."-".$newdata->delivery_time->to_time:"";
                                $arraylistA[$found_key]['timeslot']   = (!empty($newdata->delivery_time))?$newdata->delivery_time->from_time."-".$newdata->delivery_time->to_time:"";
                                $arraylistA[$found_key]['from_timestamp'] = (!empty($newdata->delivery_time))?strtotime($newdata->delivery_time->from_time):"";
                            }else{
                        ...
                        //print_r($refrenceArray);
                   
                   
                }
                //print_r($arraylistA[$newdata->id]);die;
                //usort($arraylistA, array($this,'sortByDistance')) ;
                //print_r($arraylistA);die;
                $i=0;
          ...
          foreach($orders as $order){
              ...
              $neworder[]= $order->delivery_time->id;
...
public function driverOrderDetail(Request $request){
  ...
  // print_r($orders->toArray());die;
   ...
   // print_r($orders);die;
      ...
      if($request->type==1){
        //  print_r($orders->shipping_location);die;
          //foreach($orders->shipping_location as $order){
          
          
        //  echo $orders->shipping_location->id;
        $locations= $this->deliverylocation->where(['id'=>$orders->shipping_location->id])->select("actual_address")->get()->toArray();
         // print_r($orders->shipping_location->description);die;
        ...
        if(empty($locations[0]['actual_address'])){*/
         ...
         // }
            ...
            //print_r($orders->ProductOrderItem);die;
            ...
            /*    $arraylist['product_id']=$related->vendor_product->product_id;
               ...
               */
                        if($order->is_offer=='yes'){
                            ...
                            $arraylist['discounted_price']=$related->vendor_product->price;
            ...
            $arraylist['total_amount']=($orders->total_amount+$orders->delivery_charge)-$orders->admin_discount;
            ...
            $arraylist['sub_total']=$orders->total_amount;
            ...
            $arraylist['promo_code_disc']=$orders->total_amount-$orders->offer_total;
            $arraylist['admin_discount']=$orders->admin_discount;
...
public function orderStatusChange(Request $request){
    ...
    public function shopperOrderList(Request $request) {
        ...
        if ($request->type == 1) {
            $completeArrat=array();
            $orders =  $this->order->where(['shopper_id' => $request->user_id])->where('delivery_date', '>', date('Y-m-d'))->with('ProductOrderItem')->orderBy('created_at','DESC')->get();
            ...
            $orders =  $this->order->where(['shopper_id' => $request->user_id])->with('ProductOrderItem','User','shopper','driver')->orderBy('created_at','DESC')->get();
            ...
            if (!empty($neworder)) {
                ...
                foreach ($adate_array as $dataarray) {
                    ...
                    foreach ($ordersnew as $key => $newdata) {
                        $arraylist['timeslot'] = $newdata->delivery_time->from_time."-".$order->delivery_time->to_time;
                        $arraylist['id'] = $newdata->id;
                        $arraylist['user_name'] = $newdata->shipping_location->name;
                        $arraylist['driver_id'] = $newdata->driver_id;
                        $arraylist['driver_name']= $newdata->driver->name; 
        ...
        if ((isset($request->language) && !empty($request->language)) && (isset($request->zone_id) && !empty($request->zone_id))){
            ...
            if((isset($vendorData) && !empty($vendorData))){
                ...
                $query = DB::table('vendor_products AS vp')
                    ...
                    $base_url = url('/');
                    $products = $query->select(DB::raw("CONCAT('".$base_url."/public/storage/upload/', images.name) AS image_name"),'vp.product_id','vp.price','vp.qty','vp.offer_id','p.category_id','mt.name AS measurement_class','p.measurement_value','pt.name','vp.id','off.offer_type','off.offer_value','off.to_time','off.from_time',DB::raw("
