...
<div class="date-box-area">
            {!! Form::open(['route' => 'verifyOtp','method'=>'post','class'=>'form-horizontal form-label-left validation','enctype'=>'multipart/form-data']) !!}
            <input type="hidden" class="resend_otp" name="phone_number" value="{{$phone_number}}">
            <div class="col-sm-12">
            <div class="form-group">
            <label class="form-label col-md-2"  for="otp">OTP</label>   
            <div class="col-md-1"></div>
            <div class="form-group col-md-7 {{ $errors->has('otp') ? ' has-error' : '' }}">
            {!!  Form::text('otp',  null, array('class' => 'form-control custom_input','placeholder'=>'Enter OTP')) !!}
            @if( $errors->has('otp'))
            {{ Form::filedError('otp') }}
            @endif
            </div>
            ...
            <div class="col-sm-12 text-center" id="resend-otp-wait">
                <span id="resend_otp_wait">Resend OTP in 00:30</a></span>
            </div>
