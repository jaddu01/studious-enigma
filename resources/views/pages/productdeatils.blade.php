        ...
        <div class="detail-slider-topcontent">
                <?php //print_r($products); die;
                 if($products['is_offer']){  ?>
                   ...
                   <?php } 
           if(Auth::user()){ ?>  
        <span onclick="addtowhishlist({{$products['id']}})"  class="heart-icon  @if(!empty($products['wishList'])) wishlist @endif"><i class="fa fa-heart" aria-hidden="true"></i></span>  
        <?php }else{ ?>
...
<button type="button" id="sub" onclick="addtocart({{$products['id']}},'sub')" class="sub lft_btn_qty"><img src="{{url('public/images/arrow_left.png')}}" alt="arrow"></button>
<input type="number" id="cartval{{$products['id']}}" value="{{$products['cart']['qty']}}" max='1' />
<button type="button" id="add" onclick="addtocart({{$products['id']}},'add')"   class="add rt_btn_qty"><img src="{{url('public/images/arrow_right.png')}}" alt="arrow"></button>
</div>
@if($products['qty']==0) <button  onclick="notifyMe({{$products['id']}},0)" class="btn btn-success">Notify Me</button>  @endif <?php }else{
if(Auth::user()){ ?>
@if($products['qty']==0) <b> OUT OF STOCK</b> @endif
<button @if($products['qty']==0) style="cursor: auto;" @endif @if($products['qty']>0) onclick="addtocart({{$products['id']}},1)" @endif class="add-to-card-btn">Add To Cart</button>
@if($products['qty']==0) <button  onclick="notifyMe({{$products['id']}},0)" class="btn btn-success">Notify Me</button>  @endif
<?php }else{ ?>
<a data-target="#login_with_mobile" data-toggle="modal" ><button class="add-to-card-btn">Add To Cart</button> 
</a> <?php }} ?>
</li> 
</ul>
        </div>  <hr />    
            ...
            <ul class="most-searched-slider">
                ...
                @if($products['related_products'])
                    ...
                    @if($related_product->Product)
                        <li>
                            <div class="must-have-product-box" style="{{($related_product->qty == 0)?'background-color: #fff;':''}}">
                                @php
                                ...
                                @endphp
                                 @if($related_product->qty == 0) <b style="COLOR: #000;"> OUT OF STOCK </b> @endif
                                <a href="{{ url('/product') }}/{{$related_product->id}}"><img src="{{$related_product->Product->image->name or ''}}" alt="img"></a>
                                @if(Auth::user())
                                <a class="addtowishlist"  href="javascript:;" data-data="{{$related_product->id}}">
                                    <span class="heart-icon @if($related_product->wishList) wishlist @endif">
                                        <i  onclick="addtowhishlist({{$related_product->id}})" class="fa fa-heart" aria-hidden="true"></i>
                                    </span>
                                    @else
                                     <a data-target="#login_with_mobile" data-toggle="modal" >
                                     <span   class="heart-icon">
                            ...
                            <div class="savar-product-content">
                                <a href="{{ url('/product') }}/{{$related_product->id}}">
                                    @if($related_product->offer_id)
                                        <p>₹ {{$related_product['offer_price']}} <span class="discount-price">₹ {{$related_product->price}}</span></p>
                                    @else
                                        <p>₹ {{$related_product->price}} <span class="discount-price">₹ {{$related_product->best_price}}</span></p>
                                    @endif
                                    <h4>{{$related_product->Product->name}}</h4>
                                </a>
                                @if($related_product->cart)
                                    <div id="" class="cstm_qty_inpt">
                                        <button type="button" id="sub" onclick="addtocart({{$related_product->id}},'sub')" class="sub lft_btn_qty"><img src="{{url('public/images/arrow_left.png')}}" alt="arrow"></button>
                                        <input type="number" id="cartval{{$related_product->id}}" value="{{$related_product->cart->qty}}" />
                                        <button type="button" id="add" onclick="addtocart({{$related_product->id}},'add')"   class="add rt_btn_qty"><img src="{{url('public/images/arrow_right.png')}}" alt="arrow"></button>
                                    </div>
                                @else
                                 @if($related_product->qty != 0)
                                   <button onclick="addtocart({{$related_product->id}},1)" class="add-to-card-btn">Add To Cart</button>
                                @endif
