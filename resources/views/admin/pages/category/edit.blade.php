                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="x_panel">
                        ...
                        <div class="x_content">
                            ...
                            {!! Form::model($category,['route' => ['category.update',$category->id],'method'=>'post','class'=>'form-horizontal form-label-left validation','enctype'=>'multipart/form-data']) !!}
                                {{csrf_field()}}
                                {{method_field('put')}}
                                ...
                                @foreach(config('translatable.locales') as $locale)
                                    ...
                                    <div class="item form-group {{ $errors->has('banner_image') ? ' has-error' : '' }}">
                                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="banner_image">Banner Image In {{$locale}}<span class="required">*</span>
                                        </label>
                                        <div class="col-md-6 col-sm-6 col-xs-12">
                                            <img src="/storage/app/public/upload/{{$category->{'banner_image:'.$locale} }}" height="75" width="75"/>
                                ...
                                <div class="item form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        ...
                                        <select name="parent_id" id="parent_id" class="form-control col-md-7 col-xs-12">
                                            ...
                                            @foreach($categories as $key=>$value)
                                                <option value="{{$value->id}}" class="optionGroup" {{($category->parent_id == $value->id) ? 'selected' :''}}>{{$value->name}}</option>
                                                @if(isset($value->sub_categories) && !empty($value->sub_categories))
                                                    @foreach($value->sub_categories as $key1=>$value1)
                                                        <option value="{{$value1->id}}" class="optionChild" {{($category->parent_id == $value1->id) ? 'selected' :''}}>{{$value1->name}}</option>
                                                    @endforeach
                                                @endif
                                            @endforeach
                                        </select>
                                 ...
                                 <div class="item form-group">
                                        ...
                                        <div class="col-md-6 col-sm-6 col-xs-12">
                                            <input name="is_show" type="checkbox" {{ ($category->is_show=='1') ? 'checked' : '' }} value={{ $value->is_show }}>
