...
@section('title', ' Measurement Class |')
@push('css')
...
$(function() {
    ...
    window.table=$('#users-table').DataTable({
       autoWidth:false,
        ...
        serverSide: false,
        ajax: '{!! route('measurement-class.datatable') !!}',
        fnDrawCallback :function() {
            $('.data-toggle-coustom').bootstrapToggle();
            $('.data-toggle-coustom').change(function() {
                var measurement_class =$(this).attr('measurement_class-id');
                changeStatus(measurement_class,$(this).val());
            })
        },
        columns: [
            { data: 'id', name: 'id' },
...
function deleteRow(id){
    $.ajax({
        ...
        type: "DELETE",
        url: "{{ url('admin/measurement-class') }}/"+id,
...
function changeStatus(id,status){
    $.ajax({
        ...
        type: "PATCH",
        url: "{!! route('admin.measurement-class.status') !!}",
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        success: function( data ) {
            window.table.draw();
            new PNotify({
