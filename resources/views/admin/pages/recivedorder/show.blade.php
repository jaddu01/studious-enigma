        ...
        <div class="">
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="x_panel">
								...
								<!--  <li ><a data-toggle="tab" href="#Unpaid">Order Details</a></li>
                                 <li ><a data-toggle="tab" href="#Customers">Customers</a></li>
                               -->   <li class="active" ><a data-toggle="tab" href="#Products">Pick List</a></li>                         
...
<tr><th align="center" style="text-align: center;">
    <?php
        $data = json_decode($ProductOrderItem->data,true);
        if(isset($ProductOrderItem->Product->image)){ ?>
       <img src="{{ $ProductOrderItem->Product->image['name'] }}" height="70" width="70" alt="Product" class="center" /> 
    <?php  } ?>
</th>
<th>{{$ProductOrderItem->price}}</th>
    ...
    <th><?php 
       echo '<input type="text" name="driver_name" class="driver_name" placeholder="Enter driver name here" data-id="'.$ProductOrderItem->order_id.'" value="'.$ProductOrderItem->driver_name.'" >';
      ?></th>
    <th>{{ $ProductOrderItem->Product->name }}</th>
     <?php 
        $new_order_status = ['Accepted'=>'Accepted','En Route'=>'En Route','Cancelled'=>'Cancelled'];
       if($orders_details->order_status =='N' || $orders_details->order_status =='CF' || $orders_details->order_status == 'PO' || $orders_details->order_status =='D') { ?>
        <th> <select name="order_status" class="form-control" onchange="changeItemStatus({{$ProductOrderItem->id}},$('[name=order_status]').val())">
        @foreach($new_order_status as $key=>$order_status)
...
@push('scripts')
    <script>
        function changeItemStatus(id,status){
            ...
            if(status=='En Route'){callajax=1; 
                alert(drivername);
              if(drivername=="" && drivername==null){ callajax=1;  } 
            }
            if(callajax!=0){
                alertify.error("PLease select Driver name First");
                 location.reload();
    ...
    $(document).on("focusout",".driver_name",function(){
    ...
    $.ajax({
        ...
        type: "POST",
        url: "{!! route('enroutedorder.saveDriver') !!}",
