    ...
    <script>
        ...
        $(function() {
            ...
            window.table=$('#users-table').DataTable({
               autoWidth:false,
                ...
                serverSide: false,
                ajax: '{!! route('coinSettings.datatable') !!}',
                fnDrawCallback :function() {
                    $('.data-toggle-coustom').bootstrapToggle();
                    $('.data-toggle-coustom').change(function() {
                        var coin_setting_id =$(this).attr('coin-setting-id');
                        changeStatus(coin_setting_id,$(this).val());
                    })
                },
                columns: [
                    { data: 'id', name: 'id' },
        ...
        function deleteRow(id){
            $.ajax({
                ...
                type: "DELETE",
                url: "{{ url('admin/coin-settings') }}/"+id,
        ...
        function changeStatus(id,status){
            $.ajax({
                ...
                type: "PATCH",
                url: "{!! route('admin.coinSettings.status') !!}",
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                success: function( data ) {
                    window.table.draw();
                    new PNotify({
